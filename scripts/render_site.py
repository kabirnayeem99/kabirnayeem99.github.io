...
from typing import Final, Generic, Literal, NewType, TextIO, TypeVar
...
def load_site_content(content_path: str, stdin: TextIO) -> SiteContent:
    ...
    try:
        raw_value: object = json.loads(raw_text)
...
def render_language_switcher(
    ...
    """Render the language switcher with page-aware fallbacks."""

    switcher_label = site.locales[lang].language_switcher_label
    lines = [
        '      <details class="language-switcher">',
        (
            f'        <summary aria-label="{html.escape(switcher_label)}">'
            '<span aria-hidden="true" class="language-switcher-globe"></span>'
            f'<span class="language-switcher-label">{html.escape(switcher_label)}</span>'
            "</summary>"
        ),
    ...
    for target_lang in LANGS:
        ...
        href = relative_href(current_output, route_for(routes, page_id, target_lang))
...
def render_nav(
    ...
    for nav_page_id in content.navigation[lang]:
        ...
        lines.append(f'        <a href="{html.escape(href)}">{html.escape(label)}</a>')
    lines.append("      </nav>")
    return "\n".join(lines)
...
def render_scripts(page_id: PageId, current_output: str) -> str:
    ...
    default_scripts = ("assets/js/image-guard.js", "assets/js/year.js")
    stats_scripts = (
    ...
    paths = stats_scripts if page_id == "stats" else default_scripts
    return "\n".join(
        f'  <script defer src="{html.escape(asset_href(current_output, path))}"></script>'
        for path in paths
    )
...
def render_stats_main(page: StatsPageLocale) -> str:
    ...
    lines.extend(
        (
            ...
            "          <br />",
            (
                f"          <p>{stats.languages.wakatime_copy} "
                f'<a href="{html.escape(stats.languages.wakatime_profile_href)}">'
                f'<img data-hide-section-on-error="true" src="{html.escape(stats.languages.wakatime_badge_src)}" '
                f'alt="{html.escape(stats.languages.wakatime_badge_alt)}" /></a></p>'
            ),
            (
                f'          <a href="{html.escape(stats.languages.wakatime_profile_href)}">'
                f'<img data-hide-section-on-error="true" class="stats-embed" '
                f'src="{html.escape(stats.languages.wakatime_embed_src)}" '
                f'alt="{html.escape(stats.languages.wakatime_embed_alt)}" loading="lazy" /></a>'
            ),
            "          <br />",
            f"          <p>{stats.languages.github_copy}</p>",
            (
                f'          <a href="{html.escape(stats.languages.github_profile_href)}">'
                f'<img data-hide-section-on-error="true" class="stats-embed" '
                f'src="{html.escape(stats.languages.github_embed_src)}" '
                f'alt="{html.escape(stats.languages.github_embed_alt)}" loading="lazy" /></a>'
            ),
            "        </div>",
            "      </section>",
            "      <section>",
            ...
            (
                f'          <img data-hide-section-on-error="true" class="stats-embed" '
                f'src="{html.escape(stats.leetcode.card_src)}" '
                f'alt="{html.escape(stats.leetcode.card_alt)}" loading="lazy" />'
            ),
            "        </div>",
            "      </section>",
            "      <section>",
            ...
            (
                ...
                f'<img data-hide-section-on-error="true" '
                f'src="{html.escape(stats.learning_path.image_src)}" '
                f'alt="{html.escape(stats.learning_path.image_alt)}" /></a>'
            ),
            "        </div>",
            "      </section>",
            "      <section>",
            ...
            "            }",
            "          </style>",
            f'          <div id="gr_grid_widget_{html.escape(stats.goodreads.widget_id)}"></div>',
            "        </div>",
            "      </section>",
            "    </main>",
        )
...
def render_page(content: SiteContent, page_id: PageId, lang: Lang, route: str, og_type: str) -> str:
    ...
    head = render_head(content.site, page_id, lang, route, route, meta_for_page(content, page_id, lang))
    body_lines = [
        "<body>",
        '  <div class="site">',
        "    <header>",
